 ...
 * @brief 获取套接字的选项值
 * 
 * 该函数调用系统的 getsockopt 函数来获取指定套接字的选项值。
 * 如果获取过程中出现错误，会记录调试日志并返回 false。
 * 
 * @param level 选项所在的协议层，例如 SOL_SOCKET、IPPROTO_TCP 等
 * @param option 要获取的选项名，例如 SO_RCVTIMEO、SO_SNDTIMEO 等
 * @param result 用于存储选项值的缓冲区指针
 * @param len 指向缓冲区长度的指针，调用返回后会更新为实际存储的选项值长度
 * @return bool 如果成功获取选项值，返回 true；否则返回 false
 */
bool Socket::getOption(int level, int option, void* result, socklen_t* len) {
    int rt = getsockopt(m_sock, level, option, result, (socklen_t*)len);
    // 检查 getsockopt 函数的返回值，如果不为 0 表示出现错误
    if(rt) {
        // 记录调试日志，包含套接字描述符、协议层、选项名、错误号和错误信息
 ...
 * @brief 将套接字绑定到指定的地址
 * 
 * 该函数尝试将当前套接字绑定到传入的地址。如果套接字无效，会尝试创建新的套接字。
 * 对于 Unix 地址，会先检查该地址是否已被使用，若已使用则绑定失败；若未使用则删除对应路径文件。
 * 
 * @param addr 要绑定的地址指针
 * @return bool 若绑定成功返回 true，否则返回 false
 */
bool Socket::bind(const Address::ptr addr) {
    ...
    if(uaddr) {
        ...
        // 尝试连接到该 Unix 地址，若连接成功说明地址已被使用，绑定失败
        if(sock->connect(uaddr)) {
            return false;
        } else {
            // 若连接失败，说明地址未被使用，删除该 Unix 地址对应的路径文件
            Sylar::FSUtil::Unlink(uaddr->getPath(), true);
        }
    }

    // 调用系统的 bind 函数将套接字绑定到指定地址
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if(timeout_ms == (uint64_t)-1) {
        ...
        // 处理带超时限制的连接情况，调用自定义的 connect_with_timeout 函数
        if(::connect_with_timeout(m_sock, addr->getAddr(), addr->getAddrLen(), timeout_ms)) {
            // 若连接超时或失败，记录错误日志，关闭套接字并返回连接失败
            SYLAR_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << addr->toString()
