...
/**
 * @brief 计算无符号整数 value 二进制表示中 1 的个数
 * @tparam T 无符号整数类型
 * @param value 待计算的无符号整数值
 * @return uint32_t value 中二进制 1 的个数
 */
template<class T>
static uint32_t CountBytes(T value) {
    // 初始化计数器，用于记录 value 中二进制 1 的个数
    uint32_t result = 0;
    // 循环直到 value 变为 0
    for(; value; ++result) {
        // 清除 value 中最低位的 1
...
/**
 * @brief 获取系统网络接口的地址信息，并存储到指定的 multimap 中
 * 
 * 该函数会调用系统的 getifaddrs 函数获取所有网络接口的地址信息，
 * 然后根据地址族过滤并将结果存储到 result 中。
 * 
 * @param result 用于存储网络接口地址信息的 multimap，键为接口名称，值为地址指针和前缀长度的 pair
 * @param family 地址族，如 AF_INET、AF_INET6 或 AF_UNSPEC（表示不限制地址族）
 * @return bool 如果成功获取到至少一个地址信息，则返回 true，否则返回 false
 */
bool Address::GetInterfaceAddresses(std::multimap<std::string
    ...
    try {
        ...
        for(next = results; next; next = next->ifa_next) {
                ...
                case AF_INET6:
                    {
                        ...
                        // 初始化前缀长度为 0
                        prefix_len = 0;
                        // 遍历 IPv6 网络掩码的 16 个字节，计算 1 的总个数
                        for(int i = 0; i < 16; ++i) {
            ...
            // 如果成功创建了地址对象，则将接口名称、地址指针和前缀长度存储到 result 中
            if(addr) {
